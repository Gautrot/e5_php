...
// Méthode d'affichage d'une discusssion
    ...
    public function chercheDiscussion(Discussion $discus)
    {
        ...
        $id = ['idEleve', 'idProf', 'idParent'];
        $idDis = ['idCreateurEleve', 'idCreateurProf', 'idCreateurParent'];
        // on appelle la base de données
        $bdd = (new BDD)->getBase();
        for ($i = 0; $i < 3; $i++) {
            for ($j = 0; $j < 3; $j++) {
                $req = $bdd->prepare('SELECT * FROM discussion INNER JOIN ' . $statut[$i] . ' ON ' . $statut[$i] . '.' . $id[$i] . ' = discussion.' . $idDis[$j] . ' WHERE idDiscussion = :idDiscussion');
                $req->execute([
...
// Méthode de liste de réponse
    ...
    public function listeReponse()
    {
        ...
        for ($i = 0; $i < 3; $i++) {
            for ($j = 0; $j < 3; $j++) {
                $req = $bdd->prepare('SELECT * FROM reponse INNER JOIN ' . $statut[$j] . ' ON ' . $statut[$j] . '.' . $id[$j] . ' = reponse.' . $idDis[$i] . ' WHERE idDiscussion = :idDiscussion');
                $req->execute([
                    'idDiscussion' => $res[0]
                ]);
                $res2 = $req->fetch();
